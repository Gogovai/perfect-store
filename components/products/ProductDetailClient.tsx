...
export function ProductDetailClient({ product, relatedProducts }: ProductDetailClientProps) {
  ...
  return (
    <div className="py-4 sm:py-6 bg-gray-50 min-h-[calc(100vh-8rem)]">
      <Container size="xl">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-1 text-sm text-gray-500 mb-4 overflow-x-auto" aria-label="Breadcrumb">
          ...
          {category && (
            <>
              <Link href={`/categories/${category.slug}`} className="hover:text-gray-900 shrink-0">
                {category.name}
              </Link>
              <ChevronRight size={14} className="text-gray-300 shrink-0" />
            </>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-start justify-between">
                {seller && (
                  ...
                  >
                    ...
                    {seller.rating > 0 && (
                      <span className="text-xs text-gray-400">({seller.rating.toFixed(1)}★)</span>
                    )}
                  </Link>
                )}
                <WishlistButton
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <ProductPrice
                  price={product.base_price}
                  variantPrice={selectedVariantData ? selectedVariantData.price : null}
                  size="lg"
                />
              </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-2 border rounded-lg text-sm transition-colors ${
                          selectedVariant === variant.id
                            ? 'border-[#0f2b5b] bg-blue-50 text-[#0f2b5b] font-medium'
                            : 'border-gray-300 hover:border-gray-400'
                        }`}
                      >
                        <span className="font-medium">{variant.name}</span>
                        <span className="text-gray-500 ml-2">{formatPrice(variant.price)}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Button
                  ...
                  disabled={!product.is_active || addedToCart}
                >
                  {addedToCart ? (
                    <>
                      <Check size={20} />
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-medium ${product.is_active ? 'text-green-700' : 'text-red-600'}`}>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3 border border-gray-100">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-gray-600">Free delivery on orders over ₵200</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <Shield size={18} className="text-gray-500 shrink-0" />
                  <span className="text-gray-600">Buyer protection guarantee</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
