-- Test product seed: Creates a platform-owned test product that can be used
-- to verify the full marketplace lifecycle:
-- PRODUCT → CART → CHECKOUT → PAYMENT → ORDER → COMMISSION
--
-- Run this after the main migrations are applied.

-- The admin seller record should already exist from the admin_create_product RPC.
-- We'll use a direct insert here for the seed.
...
BEGIN
  ...
  IF v_product_id IS NOT NULL THEN
    RAISE NOTICE 'Test product already exists; skipping';
    RETURN;
  END IF;
  ...
  INSERT INTO public.products (
    ...
    'This is a test product to verify the complete marketplace lifecycle. It can be added to cart, checked out, paid for via Paystack, and creates proper order/commission records.',
  ...
  RAISE NOTICE 'Test product created: %', v_product_id;
END $do$;
