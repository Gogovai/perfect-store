...
export async function updateSellerStatus(input:unknown){const p=schema.safeParse(input);if(!p.success)return{success:false,error:'Invalid seller update'};const s=await createClient();const{data:{user}}=await s.auth.getUser();if(!user)return{success:false,error:'Not authenticated'};const{data:profile}=await s.from('profiles').select('role').eq('id',user.id).single();if(profile?.role!=='admin')return{success:false,error:'Admin access required'};const{error}=await s.rpc('admin_set_seller_status',{p_seller_id:p.data.sellerId,p_status:p.data.status});if(error)return{success:false,error:error.message};revalidatePath('/admin/sellers');revalidatePath('/seller/dashboard');return{success:true};}
